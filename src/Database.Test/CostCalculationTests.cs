...
public class CostCalculationTests
{
    ...
    public void Scan()
    {
        ...
        var scan = new Scan(table.Name, table.Id, null, table.Columns);
    ...
    {
        var table = _catalog.GetTable("lineitem");
        LogicalPlan smaller = new Scan(table.Name, table.Id, null, table.Columns, Alias: "left");
        var expr = new BinaryExpression(TokenType.EQUAL, "=", new IntegerLiteral(0), new IntegerLiteral(0));
        smaller = new Filter(smaller, expr);

        var larger = new Scan(table.Name, table.Id, null, table.Columns, Alias: "right");
